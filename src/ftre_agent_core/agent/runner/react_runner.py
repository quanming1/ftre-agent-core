...
架构分三层：
  ...
  _decide()              纯决策函数，只读状态，返回动作类型
  _execute_reasoning     执行 Reasoning 动作：调 LLM + 流式 + 重试
  _execute_acting        执行 Acting 动作：工具并发 + 成组写入 Memory
  _execute_exit          执行 Exit 动作：ON_STOP Hook + 产出 ReplyEnd
...
主循环 _loop() 只做 match 分发：
  ...
  while True:
      ...
      match action:
          Reasoning → prev = execute_reasoning(action)
          Acting    → execute_acting(action); prev = None
          Exit      → execute_exit(action); return
