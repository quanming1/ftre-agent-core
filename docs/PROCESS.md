# ftre-agent-core 项目推进管理办法（PROCESS.md）

> 本文件定义 ftre-agent-core 的开发推进机制：**先 PRD，后开发**。任何阶段没有定稿的 PRD 不开工。
...
| 4. 验证 | 对照 PRD「验收标准」逐条执行（pytest / ruff / 手动） | 全部通过 → 进入收尾；失败 → 回开发 |
...
- **位置**：`docs/prd/`
- **命名**：`PRD-<阶段>-<名称>.md`，如 `PRD-A2-tool-system.md`
...
- 验收三步：`pytest`（自动化测试）+ `ruff`（代码规范）+ PRD 手动验收项。
